...
from app.services.asset_mention_service import AssetMentionService

router = APIRouter()


@router.post("/{note_id}/assets/sync")
async def sync_note_assets(
    note_id: UUID,
    current_user: dict = Depends(get_current_user)
):
    """Extract asset names/codes from a note and refresh its normalized tags."""
    service = AssetMentionService()
    mentions = await service.sync_note(str(note_id), current_user["user_id"])
    return {"mentions": mentions}


@router.post("/assets/backfill")
async def backfill_note_assets(
    current_user: dict = Depends(get_current_user),
    limit: int = Query(100, ge=1, le=500)
):
    """Backfill asset tags for existing notes owned by the current user."""
    ...
    total_mentions = 0
    for note in notes:
        mentions = await service.sync_note(str(note.id), current_user["user_id"])
        total_mentions += len(mentions)
    return {"notes_processed": len(notes), "mentions_created": total_mentions}
...
@router.patch("/{note_id}", response_model=ReflectionNoteResponse)
async def update_note(
    note_id: UUID,
    note_data: ReflectionNoteUpdate,
    current_user: dict = Depends(get_current_user)
):
    """Update a specific note's reflection content"""
    note_service = NoteService()
